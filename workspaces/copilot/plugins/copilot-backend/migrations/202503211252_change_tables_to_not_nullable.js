...
exports.up = async function up(knex) {
  ...
  // then delete all that are left with null in team_name
  await knex('copilot_metrics').where('team_name', null).delete();

  // if we found any rows, insert a new row with the same day, type and team_name = ''
  if (rows.length > 0) {
  ...
  await knex('ide_completions').where('team_name', null).delete();
  ...
  await knex('ide_chats').where('team_name', null).delete();
  ...
  await knex('ide_chat_editors').where('team_name', null).delete();
  ...
  await knex('ide_chat_editors_model').where('team_name', null).delete();
  ...
  await knex('dotcom_chats').where('team_name', null).delete();
  ...
  await knex('dotcom_chat_models').where('team_name', null).delete();
  ...
  await knex('dotcom_prs').where('team_name', null).delete();
  ...
  await knex('dotcom_pr_repositories').where('team_name', null).delete();
  ...
  await knex('dotcom_pr_repositories_models').where('team_name', null).delete();
